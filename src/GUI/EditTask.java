...
public class EditTask extends JFrame implements ActionListener, MouseListener, KeyListener {
    ...
    EditTask(String str, JLabel label){

        // here since a referance of the label is passed hence the referance passed is used to edit label
        temp=label;
        taskAlready = str;
    ...
    public void mousePressed(MouseEvent e) {
        if(e.getSource()== taskSubmitButton){
            ...
            int rep=BufferReading.tasks.indexOf(taskAlready);

            if( !taskEnter.getText().equals("")&&!BufferReading.tasks.contains(taskHere)) {
                BufferReading.tasks.remove(rep);
                BufferReading.tasks.add(rep,taskHere);
                try {
                    Writing.write();
                } catch (IOException ex) {
                    ex.printStackTrace();
                }
                temp.setText(taskHere); //referance passed is used
